...
public:
    ...
    void loadBooks() {
        ...
        if (infile.is_open()) {
            Book book;
            while (infile >> book.name >> book.author >> book.pages >> book.yearReleased >> book.availableCopies) {
    ...
    void viewBookList() {
        cout << setw(20) << "Name" << setw(20) << "Author" << setw(10) << "Pages"
             << setw(15) << "Year Released" << setw(15) << "Available Copies" << endl;
        cout << "------------------------------------------------------------------------------" << endl;
        for (const auto& book : library) {
            cout << setw(20) << book.name << setw(20) << book.author << setw(10) << book.pages
